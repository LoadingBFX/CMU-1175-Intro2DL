...
def get_optimizer(model, config):
    ...
    if config["optimizer"] == "SGD":
        ...
        # feel free to change any of the initializations you like to fit your needs
        optimizer = torch.optim.AdamW(model.parameters(), lr=float(config["learning_rate"]), weight_decay=0.01)
    return optimizer
...
def get_scheduler(optimizer, config):
    ...
    if config["scheduler"] == "ReduceLR":
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=config["factor"],
                                                               patience=config["patience"], min_lr=1e-8)
