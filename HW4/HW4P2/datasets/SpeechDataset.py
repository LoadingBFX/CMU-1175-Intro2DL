...
class SpeechDataset(Dataset):
    ...
    def __init__(self, partition, config, tokenizer, isTrainPartition):
        self.config = config
        ...
        self.pad_token_id = tokenizer.pad_id  # 确保填充值为数值ID
        ...
        self.fbank_files = sorted(os.listdir(self.fbank_dir))[:int(self.subset * len(os.listdir(self.fbank_dir)))]

        if partition != 'test-clean':
            self.text_dir = os.path.join(self.root, self.partition, "text")
            self.text_files = sorted(os.listdir(self.text_dir))[:len(self.fbank_files)]
            assert len(self.fbank_files) == len(self.text_files), "Mismatch in fbank and text files"
    ...
    def __getitem__(self, idx):
        feat_path = os.path.join(self.fbank_dir, self.fbank_files[idx])
        feat = np.load(feat_path)

        if self.feat_type == 'mfcc':
            feat = self.fbank_to_mfcc(feat)
        if self.norm == 'cepstral':
            feat = (feat - np.mean(feat, axis=0)) / (np.std(feat, axis=0) + 1e-8)
        feat = torch.FloatTensor(feat[:self.num_feats, :])
        ...
        if self.partition != 'test-clean':
            transcript_path = os.path.join(self.text_dir, self.text_files[idx])
    ...
    def collate_fn(self, batch):
        ...
        lengths_feats = [feat.shape[0] for feat in batch_feats]
        batch_feats_pad = pad_sequence(batch_feats, batch_first=True, padding_value=0)

        if self.partition != 'test-clean':
            batch_transcripts = [item[1] for item in batch]
            batch_golden = [item[2] for item in batch]
            lengths_transcripts = [len(transcript) for transcript in batch_transcripts]
            batch_transcripts_pad = pad_sequence(batch_transcripts, batch_first=True, padding_value=self.pad_token_id)
            batch_golden_pad = pad_sequence(batch_golden, batch_first=True, padding_value=self.pad_token_id)

            return batch_feats_pad, batch_transcripts_pad, batch_golden_pad, torch.tensor(lengths_feats), torch.tensor(
                lengths_transcripts)
        else:
    ...
    def compute_global_stats(self):
        print("Computing global mean and variance...")
        sum_feats, sum_squares, total_frames = 0, 0, 0
        for fbank_file in self.fbank_files:
            feats = np.load(os.path.join(self.fbank_dir, fbank_file))
            sum_feats += np.sum(feats, axis=1)
            sum_squares += np.sum(feats ** 2, axis=1)
            total_frames += feats.shape[1]
        mean = sum_feats / total_frames
        variance = (sum_squares / total_frames) - (mean ** 2)
        std_dev = np.sqrt(variance + 1E-8)
        return torch.FloatTensor(mean), torch.FloatTensor(std_dev)
...
#         self.fbank_files = self.fbank_files[:subset]
#
#         if self.partition != 'test-clean':
#             self.text_dir = os.path.join(self.root, self.partition, "text")
...
#         batch_feats_pad = pad_sequence(batch_feats, batch_first=True, padding_value=self.pad_token)  # Pad sequence
#
#         if self.partition != 'test-clean':
