...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout):
        ...
        self.self_attn = nn.MultiheadAttention(d_model, num_heads, dropout=dropout, batch_first=True).to(device)
    ...
    def forward(self, x, pad_mask):
        ...
        x_norm = self.pre_norm(x).to(device)

        # Step 2: Self-attention with with dropout, and with residual connection
        attn_output, _ = self.self_attn(x_norm, x_norm, x_norm, key_padding_mask=pad_mask, need_weights=False)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_len):
        ...
        mask = PadMask(x, x_len).to(device)
