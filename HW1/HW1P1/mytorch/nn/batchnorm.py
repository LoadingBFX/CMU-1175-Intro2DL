...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = np.sum(dLdNZ * (self.Z - self.M) * -0.5 * (self.V + self.eps) ** (-1.5), axis=0)  # TODO
        dLdM = np.sum(dLdNZ * -1 / np.sqrt(self.V + self.eps), axis=0) + dLdV * np.mean(-2 * (self.Z - self.M), axis=0)  # TODO
