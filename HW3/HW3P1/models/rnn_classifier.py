    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for i in range(self.num_layers):
                ...
                x_input = h_ti  # Input for the next layer
                hidden.append(h_ti)
            self.hiddens.append(hidden.copy())
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len - 1, -1, -1):
            for i in range(self.num_layers - 1, -1, -1):
                ...
                # Backpropagate through the RNN cell
                dx, dh_new = self.rnn[i].backward(dh[i], self.hiddens[t + 1][i], h_prev_l, h_prev_t)
                dh[i] = dh_new  # Update dh for the next time step

                # Accumulate gradients if not in the first layer
                if i != 0:
                    dh[i - 1] += dx
